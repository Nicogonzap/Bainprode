  ...
  return (
    <div className="rounded-md p-6" style={{ backgroundColor: BAIN.white, border: `1px solid #FFD54F` }}>
      ...
      <div className="flex gap-3">
        <button type="button" onClick={handleAccept} disabled={accepting || rejecting}
        ...
        <button type="button" onClick={handleReject} disabled={accepting || rejecting}
          className="flex-1 py-2 rounded-md text-sm font-bold transition-opacity"
  ...
  return (
    <div className="rounded-md p-6" style={{ backgroundColor: BAIN.white, border: `1px solid ${BAIN.grayBorder}` }}>
      <div className="flex items-start justify-between gap-4 mb-5">
        ...
        {card.id !== null && (
          <Link href={`/torneo/${card.id}`} className="text-xs font-medium flex-shrink-0 underline"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            ...
            <input
              ...
              placeholder="Ej: Prode Bain Chile"
              maxLength={80}
              required
              className="w-full px-3 py-2.5 rounded-md text-sm outline-none focus:ring-2"
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={300}
              className="w-full px-3 py-2.5 rounded-md text-sm outline-none focus:ring-2 resize-none"
              style={{ border: `1px solid ${BAIN.grayBorder}`, color: BAIN.black, backgroundColor: BAIN.white }}
            />
          </div>
...
function MisTorneosContent() {
  ...
  const handleAccept = async (torneoId: string) => {
    if (!user) return
  ...
  const handleReject = async (torneoId: string) => {
    if (!user) return
    const res = await fetch('/api/torneos/accept', {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleCreated = async (torneo: any) => {
    setShowModal(false)
    setLoading(true)
    setCards([])
    setPendingCards([])
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: BAIN.grayBg }}>
      ...
      <main className="flex-1 max-w-[1200px] w-full mx-auto px-6 py-10">
        <section className="flex items-end justify-between mb-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div>
            <p className="text-xs font-bold uppercase mb-2" style={{ color: BAIN.red, letterSpacing: '0.1em' }}>
              TORNEOS
            </p>
      ...
      <Footer />
      {showModal && <CrearTorneoModal onClose={() => setShowModal(false)} onCreated={handleCreated} />}
    </div>
  )
}
